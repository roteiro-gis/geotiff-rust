...
impl TiffFile {
    ...
    /// Decode an image into native-endian interleaved pixel bytes.
    pub fn read_image_bytes(&self, ifd_index: usize) -> Result<Vec<u8>> {
        let ifd = self.ifd(ifd_index)?;
        self.read_image_bytes_from_ifd(ifd)
    }

    /// Decode an arbitrary IFD into native-endian interleaved pixel bytes.
    ...
    /// Decode a pixel window into native-endian interleaved pixel bytes.
    ...
    fn decode_window_pixel_bytes(&self, ifd: &Ifd, window: Window) -> Result<Vec<u8>> {
        ...
        let (_, pixels) =
            pixel::decode_pixels(ifd, &storage_layout, window.cols, window.rows, &sample_bytes)?;
        Ok(pixels)
    }
    ...
    /// Multi-channel decoded rasters are returned as shape `[height, width, channels]`.
...
mod tests {
    ...
    #[test]
    fn decodes_subbyte_palette_pixels_and_exposes_unpacked_sample_bytes() {
        ...
        let data = build_stripped_tiff(
            ...
            &[
                ...
                (320, 3, color_map.len() as u32, color_map.iter().flat_map(|value| le_u16(*value)).collect()),
            ],
        );
        let file = TiffFile::from_bytes(data).unwrap();
    ...
    #[test]
    fn decodes_subsampled_ycbcr_pixels_and_preserves_sample_reads() {
        let data = build_stripped_tiff(
            2,
            2,
            &[10u8, 20, 30, 40, 128, 128],
            &[
                (258, 3, 3, [8u16, 8, 8].into_iter().flat_map(le_u16).collect()),
